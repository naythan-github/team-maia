...
### **Minecraft Education Agent v1.0** ⭐ **NEW - CHILDREN'S LEARNING GAME EXPERT**
The Minecraft Education Agent guides children's learning through Minecraft with age-appropriate instruction in creativity, problem-solving, redstone logic, building design, and collaborative play:
- **Core Specialties**: Building & Design (architecture basics, spatial reasoning, aesthetic principles), Redstone Logic (digital circuits, problem-solving, cause-and-effect), Resource Management (planning, organization, goal-setting), Creativity (self-expression, storytelling, world-building), Collaboration (teamwork, communication, sharing ideas)
- **Age-Appropriate Communication**: Simple clear language with visual descriptions, growth mindset celebrating attempts and learning not just success, frames failures as experiments, safety-first approach with creative mode for learning, positive multiplayer behavior, time management and breaks
- **Educational Objectives**: STEM Learning (redstone logic AND/OR gates, math counting/area/perimeter, engineering problem decomposition, spatial reasoning 3D visualization), Creative Skills (design aesthetics/color theory, storytelling world narratives, art pixel art/sculptures), Life Skills (planning breaking big projects into steps, persistence learning from failures, collaboration teamwork/communication, time management setting goals/taking breaks)
- **Key Commands**: `building_project` (guide creative construction with skill_level/project_idea/learning_goals), `redstone_tutorial` (teach logic circuits for doors/farms/etc with complexity_level), `problem_solver` (help overcome challenges with stuck_point/what_tried/desired_outcome), `learning_quest` (design educational activities for age_range/subject/duration)
- **Few-Shot Examples**: (1) Redstone learning automatic door (celebrates attempt, diagnoses together missing redstone dust connection, teaches power path concept button→wire→door, extends learning with two-button challenge, self-reflection on encouraged attempt/taught core concept/guided discovery/extended learning), (2) Creative building project castle (explores child's vision towers/drawbridge, breaks down into achievable steps foundation→walls→towers→drawbridge, teaches foundation-first architectural principle, guides enthusiasm toward proper sequence, self-reflection on respected vision/taught principle/made fun/age-appropriate motivation)
- **Problem-Solving Approach**: Phase 1 Connect (validate their idea, ask about vision, find learning opportunity), Phase 2 Teach Concepts (explain ONE core principle clearly with real-world analogy), Phase 3 Guide Discovery (ask questions leading to solutions, celebrate discoveries), Phase 4 Extend Learning (optional challenge or new concept to explore next)
- **Learning Domains**: Ages 6-8 (basic building, color/pattern, simple redstone buttons/doors, resource gathering), Ages 9-11 (architecture, redstone circuits, planning, collaboration, basic command blocks), Ages 12+ (complex redstone, game design, server management, modding introduction)
- **Safety & Positive Play**: Online Safety (parent-approved servers only, no personal info sharing, report inappropriate behavior), Healthy Gaming (breaks every 45-60min, outdoor/offline balance, respect parental limits), Positive Reinforcement ("creative thinking!", "different approach!", "learning like real engineer!", "mistake taught something important!")
- **Parent Reports**: Learning summary (concept learned, skills practiced logic/building/creativity/collaboration, project built, next learning goal, recommended challenge) for parent/educator coordination
- **Platform Guidance**: iPad Bedrock (best for ages 6-9, touchscreen intuitive, portable, parental controls, limited redstone/no mods), Windows Java (best for ages 10+, advanced learning full redstone, educational mods ComputerCraft/WorldEdit, larger screen complex builds, command blocks game design)
- **Integration**: Works with parents/educators on educational goals, coordinates with teachers on STEM integration, UI Systems Agent for educational dashboards
- **Common Use Cases**: Redstone tutorial (automatic doors teaching power paths), building projects (castles teaching foundation-first), problem-solving (stuck on challenge guiding discovery), learning quests (STEM objectives through gameplay), platform selection (iPad vs Windows for age/goals)
- **Business Impact**: Educational value (STEM + creative + life skills embedded in gameplay), age-appropriate instruction (6-8, 9-11, 12+ tailored approaches), positive reinforcement (growth mindset learning from failures), parent visibility (learning summaries coordinating with educational goals)
- **v1.0 Production**: 285 lines, 2 comprehensive few-shot examples (redstone automatic door teaching power path concept, castle building project teaching foundation-first principle), child-focused behavior principles (encouragement/growth mindset, age-appropriate language, safety-first, learning through play), three age ranges with domain-specific guidance, safety guidelines and parent reporting

### **Men's Cycling Clothing Specialist Agent v1.0** ⭐ **NEW - CYCLING APPAREL EXPERT**
The Men's Cycling Clothing Specialist Agent provides expert product research and recommendations for cycling apparel with technical comparisons, fit guidance, and value analysis across all riding disciplines:
- **Apparel Categories**: Bibs/shorts, jerseys (short/long sleeve), base layers, jackets, vests, arm/leg warmers, gloves, socks with category-specific technical requirements
- **Technical Knowledge**: Chamois construction (single/multi-density, perforated, peak thickness 9-13mm), fabric technology (merino wool temperature regulation, synthetic blends fast-drying, Gore Windstopper windproof, DWR water repellent coating), aerodynamics and compression features, moisture management systems
- **Fit Systems**: Race fit (tight/aerodynamic), endurance/regular fit (comfortable all-day), relaxed/club fit (non-competitive), brand sizing differences (Italian brands Castelli/Santini size up, Swiss Assos size up 1-2, US Pearl Izumi same/down, UK Rapha baseline)
- **Brand Expertise**: Premium tier ($150-300 Rapha/Assos/Castelli top-line/Pas Normal), mid-tier ($80-150 Castelli mid-line/Pearl Izumi/Gore/Sportful), value tier ($40-80 Decathlon Van Rysel/DHB/The Black Bibs/Amazon brands) with price-to-performance positioning
- **Seasonal Strategy**: Summer 70°F+ (lightweight/breathable/mesh panels), spring/fall 50-70°F (arm warmers/vests/light jackets layering), winter 30-50°F (base layers/thermal jerseys/insulated bibs), deep winter <30°F (heavyweight/windproof/insulated gloves)
- **Few-Shot Examples**: (1) Summer bib shorts recommendation (100+ mile rides hot/humid, $150-200 budget, 5 options compared: Assos Mille GT best overall $199 13mm chamois, Rapha Core best value $160 comfortable fit, Pearl Izumi PRO best fit flexibility $170 roomier/moisture-wicking, technical comparison chamois/fabric/fit/pros/cons), (2) Winter riding system (30-50°F temps 2-3 hour rides $400 budget, complete 5-piece kit: Smartwool base $60 + Pearl Izumi thermal jersey $120 + Gore bibs $100 + Castelli gloves $45 + GripGrab shoe covers $30 = $370 with temperature guide and layering flexibility), (3) Brand fit comparison (6'1" 185lbs Rapha L baseline, Castelli XL conversion Italian race fit, Assos XL/XXL Swiss runs small, size conversion guide with fit philosophy notes)
- **Problem-Solving Approach**: Phase 1 Requirements Gathering (<5min riding type, climate, budget, fit preferences), Phase 2 Research & Analysis (<15min current products, technical specs, test frequently web search), Phase 3 Recommendations (<10min top options, comparisons, purchase guidance with self-reflection checkpoint on use case match/budget/technical comparison/fit guidance/alternatives)
- **Domain Reference**: Chamois technology (single-density 1-2hr, multi-density 3-5hr with variable thickness zones, perforated hot weather breathability), fabric tech (merino odor resistance/temperature regulation base layers, synthetics faster drying/durability jerseys, Gore Windstopper jackets, DWR rain protection), fit systems (race/endurance/relaxed with brand variances), seasonal temp ranges (summer 70°F+, transition 50-70°F, winter 30-50°F, deep <30°F)
- **Integration**: Works with Financial Advisor Agent (annual cycling clothing budget planning), Personal Assistant Agent (tracking orders and returns), UI Systems Agent (product comparison tables)
- **Common Search Patterns**: Category + use case + budget recommendation `"summer bib shorts endurance rides $150-200"`, brand fit comparison `"Rapha Large to Castelli sizing conversion"`, seasonal system build `"winter riding kit 30-50°F complete wardrobe $400"`, technical comparison `"chamois thickness multi-density vs single-density comfort"`, value analysis `"best cycling bibs under $100"`
- **Business Impact**: Informed purchase decisions (technical comparisons vs marketing claims), value optimization (price-to-performance analysis across tiers), fit risk reduction (brand sizing guidance reduces returns), seasonal planning (complete wardrobe systems vs ad-hoc purchases), time savings (aggregated research vs manual comparison across multiple retailers)
- **Real-World Use Cases**: Summer century ride gear selection (hot/humid conditions long-distance chamois priority), winter commuting wardrobe (layering system for variable temps), brand sizing conversion (known fit translated to new brands), budget-conscious value hunting (mid-tier performance at value prices), complete seasonal wardrobe build (spring/summer/fall/winter comprehensive kits)
- **v1.0 Production**: 450+ lines, 3 comprehensive few-shot examples (summer bib shorts with 5-option technical comparison, winter system with complete kit budget allocation, brand fit comparison with size conversion guide), all 5 v2.3 advanced patterns (self-reflection on use case/budget/technical/fit/alternatives, test frequently with web search, checkpoint on recommendation quality, prompt chaining for complete wardrobe builds, explicit handoff for budgeting/scheduling), technical domain reference (chamois types, fabric technology, fit systems, seasonal ranges)

### **Healthcare Provider Search Agent v1.0** ⭐ **NEW - MEDICAL PROFESSIONAL FINDER**
The Healthcare Provider Search Agent provides multi-source healthcare provider search and verification with cross-verified credentials, reviews, and availability across Australia:
- **Credential Verification**: AHPRA registration check (active status, qualifications, conditions/undertakings), specialty qualifications (FRACS surgeons, FRACP physicians, FRACGP GPs), board certifications and subspecialties (e.g., Mohs surgery, pediatric cardiology, rhinology)
- **Multi-Source Search Strategy**: Primary sources (AHPRA register for official credentials, healthdirect.gov.au for government-verified provider directory with bulk billing/services/languages, Google Maps for reviews/location/hours/proximity), secondary sources (RateMDs for detailed patient reviews on communication/wait times/bedside manner, clinic websites for subspecialties/team/procedures/new patient status)
- **Quality Assessment**: Patient reviews and ratings (4.5+ stars with 50+ reviews = reliable), hospital affiliations (teaching hospitals RPA/Alfred/RMH = higher standards), years of experience and procedure volume, red flags detection (AHPRA conditions, consistent negative review themes, very short wait times indicating low demand)
- **Availability Intelligence**: Clinic locations and contact information, accepting new patients status, bulk billing vs private only, wait times (typical ranges 1-2 weeks private, 4-10 weeks public), best contact methods (phone/email/online booking)
- **Healthcare Navigation**: Public vs private system (Medicare eligibility, bulk billing for eligible specialists), specialist referral requirements (GP referral required, valid 12 months), insurance compatibility (private health coverage reducing wait times)
- **Search Sources**: AHPRA (https://www.ahpra.gov.au/registration/registers-of-practitioners.aspx for credential verification), healthdirect (government-verified directory), Google Maps (reviews/location), RateMDs (detailed patient reviews), clinic websites (direct practice information)
- **Few-Shot Examples**: (1) ENT surgeon Sydney search (127 AHPRA-registered ENT surgeons filtered to 43 accepting new patients, top 5 verified with credentials/reviews/contact: Dr. Niranjan Sritharan 4.9/5 rating at Kingswood/Concord 3-6mo wait, A/Prof Narinder Singh at Bella Vista specialized rhinology/sinus surgery 4-6wk wait, Dr. Ronald Chin at Penrith head/neck cancer/robotic surgery, recommendations based on specialty needs), (2) Dermatologist comparison Melbourne (side-by-side comparison of 3 providers with AHPRA verification, review aggregation, subspecialty differentiation: Dr. Smith cosmetic focus 4.3/5 1-2wk wait, Dr. Jones medical dermatology 4.7/5 6-8wk wait with bulk billing, Dr. Lee skin cancer specialist 4.9/5 4-5wk wait with Mohs surgery fellowship, recommendations by use case)
- **Problem-Solving Approach**: Phase 1 Credential Verification (<10min AHPRA search, registration status, qualifications), Phase 2 Multi-Source Search (<15min healthdirect, Google, reviews, test frequently with cross-verification), Phase 3 Quality Assessment (<10min reviews/ratings/subspecialties with self-reflection checkpoint on quality indicators), Phase 4 Availability & Contact (<10min clinic info, new patient status, contact details)
- **Domain Reference**: Australian healthcare system (AHPRA official register, Medicare public system with bulk billing eligibility, private health reducing wait times, GP referrals required valid 12 months), credential verification (AHPRA active status with no conditions, fellowship designations FRACS/FRACP/FRACGP, subspecialty certifications), quality indicators (experience years/procedure volume, teaching hospital affiliations, 4.5+ stars with 50+ reviews, red flags for AHPRA conditions or consistent negative themes)
- **Medical Disclaimer**: Provides provider search and credential verification only, does NOT provide medical advice/diagnosis/treatment recommendations, does NOT guarantee provider quality (reviews are patient opinions), always verify credentials directly via AHPRA and consult GP for referrals
- **Integration**: Works with Personal Assistant Agent (appointment scheduling after provider selection), Financial Planner Agent (insurance coverage analysis, private health optimization)
- **Common Search Patterns**: Specialist search by location `specialty + city + AHPRA registered`, review aggregation `doctor name + specialty + city + reviews RateMDs Google`, credential verification `doctor name + AHPRA registration check`, availability check `practice name + location + contact + accepting new patients`
- **Business Impact**: Multi-source verification (cross-checking AHPRA/healthdirect/reviews for high confidence), quality filtering (ratings/reviews/credentials for informed decisions), time savings (aggregated search vs manual research across multiple sites), reusable for any medical specialty/location (GPs, specialists, dentists, etc.)
- **Real-World Use Cases**: ENT surgeon search Sydney (sinus/rhinology specialists), dermatologist comparison Melbourne (cosmetic vs medical vs skin cancer), GP search with bulk billing (primary care affordability), specialist referrals (cardiologist, orthopedic, oncology), multi-specialty care coordination (>3 specialists for complex conditions)
- **v1.0 Production**: 350+ lines, comprehensive few-shot examples (ENT surgeon Sydney multi-source search with top 5 verified providers, dermatologist comparison Melbourne with side-by-side analysis), all 5 v2.3 advanced patterns (self-reflection on quality indicators, test frequently with cross-verification, checkpoint on credential validation, prompt chaining for complex multi-specialty searches, explicit handoff for appointment scheduling), medical disclaimer for ethical boundaries
