#!/usr/bin/env python3
"""
PMP Complete Intelligence Extractor v4 - Resume Capability

Features:
- Fixed 25-record pagination (API limitation)
- Resume from last successful page on rate limit/failure
- Checkpoint tracking per endpoint
- Graceful rate limit handling

Database: ~/.maia/databases/intelligence/pmp_config.db

Usage:
    python3 pmp_complete_intelligence_extractor_v4_resume.py
"""
...
class PMPCompleteIntelligenceExtractor:
    """Comprehensive PMP data extraction with resume capability (v4)"""
    ...
    def __init__(self):
        ...
        self.PAGE_SIZE = 25  # Fixed by API
    ...
    def get_existing_record_count(self, table: str) -> int:
        """Get count of existing records in table for current extraction"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()

        try:
            cursor.execute(f"SELECT COUNT(*) FROM {table} WHERE extraction_id = ?", (self.extraction_id,))
            count = cursor.fetchone()[0]
            conn.close()
            return count
        except sqlite3.OperationalError:
            # Table might not have extraction_id column or be empty
            conn.close()
            return 0

    def calculate_resume_point(self, table: str, endpoint_name: str) -> tuple[int, int]:
        """
        Calculate resume point for an endpoint.

        Returns:
            (start_page, existing_count) - Page number to start from and existing record count
        """
        existing_count = self.get_existing_record_count(table)

        if existing_count == 0:
            print(f"   📌 Starting from beginning (no existing records)")
            return (1, 0)
        else:
            # Calculate which page to resume from
            # If we have 2,500 records, that's 100 complete pages, so resume from page 101
            start_page = (existing_count // self.PAGE_SIZE) + 1
            print(f"   📌 Resuming from page {start_page:,} ({existing_count:,} existing records)")
            return (start_page, existing_count)
    ...
    def fetch_all_pages(self, endpoint: str, data_key: str, extract_fn, table: str,
        ...
        while True:
            ...
            # Get total count on first page of THIS extraction (not necessarily page 1)
            total_count = data.get('total', 0)
            if page == start_page and total_count > 0:
                expected_pages = (total_count + self.PAGE_SIZE - 1) // self.PAGE_SIZE
                remaining_records = total_count - existing_count
                print(f"      Total records: {total_count:,} ({expected_pages} pages @ 25/page)")
                if existing_count > 0:
                    print(f"      Remaining: {remaining_records:,} records ({expected_pages - start_page + 1} pages)")
    ...
    def extract_endpoint_paginated(self, name: str, endpoint: str, table: str,
        ...
        Returns:
            Number of records extracted (total, including existing)
        """
        print(f"\n📥 Extracting: {name}")
