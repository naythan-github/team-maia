...
```python
# Automatic validation in save_state.py
save_state = SaveState()
result = save_state.validate_repository()  # Raises ValueError on mismatch

# Force override (emergency only)
result = save_state.validate_repository(force=True)  # Logs warning but proceeds
```
