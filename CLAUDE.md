...
8. 🚨 **SAVE STATE PROTOCOL** ⭐ **PHASE 233 ENFORCED**: When user says "save state", run `python3 claude/tools/sre/save_state.py` - Script auto-detects changes, BLOCKS commit if docs missing (new tools → capability_index.md, significant work → SYSTEM_STATE.md), syncs capabilities.db, then commits/pushes - Use `--check` to preview, `--force` to bypass (emergency only)
